...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'AppUser.first_name'
        db.delete_column('MainApp_appuser', 'first_name')

        # Deleting field 'AppUser.last_name'
        db.delete_column('MainApp_appuser', 'last_name')

        # Deleting field 'AppUser.created'
        db.delete_column('MainApp_appuser', 'created')

        # Deleting field 'AppUser.is_active'
        db.delete_column('MainApp_appuser', 'is_active')

        # Deleting field 'AppUser.updated'
        db.delete_column('MainApp_appuser', 'updated')

        # Deleting field 'AppUser.password_salt'
        db.delete_column('MainApp_appuser', 'password_salt')

        # Deleting field 'AppUser.nickname'
        db.delete_column('MainApp_appuser', 'nickname')

        # Deleting field 'AppUser.email'
        db.delete_column('MainApp_appuser', 'email')

        # Deleting field 'AppUser.password_hash'
        db.delete_column('MainApp_appuser', 'password_hash')


        # Changing field 'AppUser.user'
        db.alter_column('MainApp_appuser', 'user_id', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['auth.User'], unique=True))
        # Adding unique constraint on 'AppUser', fields ['user']
        db.create_unique('MainApp_appuser', ['user_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'AppUser', fields ['user']
        db.delete_unique('MainApp_appuser', ['user_id'])

        # Adding field 'AppUser.first_name'
        db.add_column('MainApp_appuser', 'first_name',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=255, blank=True),
                      keep_default=False)

        # Adding field 'AppUser.last_name'
        db.add_column('MainApp_appuser', 'last_name',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=255, blank=True),
                      keep_default=False)


        # User chose to not deal with backwards NULL issues for 'AppUser.created'
        raise RuntimeError("Cannot reverse this migration. 'AppUser.created' and its values cannot be restored.")
        # Adding field 'AppUser.is_active'
        db.add_column('MainApp_appuser', 'is_active',
                      self.gf('django.db.models.fields.BooleanField')(default=True),
                      keep_default=False)


        # User chose to not deal with backwards NULL issues for 'AppUser.updated'
        raise RuntimeError("Cannot reverse this migration. 'AppUser.updated' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'AppUser.password_salt'
        raise RuntimeError("Cannot reverse this migration. 'AppUser.password_salt' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'AppUser.nickname'
        raise RuntimeError("Cannot reverse this migration. 'AppUser.nickname' and its values cannot be restored.")
        ...
        # User chose to not deal with backwards NULL issues for 'AppUser.password_hash'
        raise RuntimeError("Cannot reverse this migration. 'AppUser.password_hash' and its values cannot be restored.")

        # Changing field 'AppUser.user'
        db.alter_column('MainApp_appuser', 'user_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User']))
