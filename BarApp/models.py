...
class BarOrder(BaseModel):
    ...
    (1, 'UNFILLED'), (2, 'IN PROGRESS'), (3, 'WAITING CUSTOMER'), (4, 'ORDER COMPLETE'), (5, 'ORDER CANCELLED'),
    (6, 'ORDER NOT CLAIMED'))
    ...
    def updateReady(self):
        uahelper = AirshipHelper()
        uahelper.pushMessageForUser(message='Your order is ready! Go get it and DrinkUp!',
                                    user=self.appuser.user, status=3)
    ...
    def voidHold(self):
        ...
        uahelper = AirshipHelper()
        uahelper.pushMessageForUser(message='Your order was cancelled. No worries, we still like you!',
                                    user=self.appuser.user, status=5)
