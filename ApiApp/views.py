...
def CurrentLocation (request):
	if request.method == 'GET':
		...
		url_request = url = 'http://maps.googleapis.com/maps/api/geocode/json?latlng=' + str(lat) + ',' + str(
			long) + '&sensor=false'
...
def UserPasswordReset (request, template='registration/password_reset_email.html'):
	if request.method == 'POST':
		...
		if email is not None:
			...
			if form.is_valid():
				form.clean()
				form.save(from_email=settings.EMAIL_SENDER_PREFIX + " <team@letsdrinkup.com>", email_template_name=template,
	...
	if request.method == 'POST': #and request.user.is_authenticated():
		...
		if bar_id and total and tax and sub_total and tip and fees and grand_total and drinks:
			...
			new_order = BarOrder(user_id=primary_user, venue=bar.venue, bar=bar, venue_name=bar.venue.name,
			                     ...
			                     grand_total=grand_total, current_status=1, description=description)
			new_order.save()
...
def GetOrdersForBarWithStatus (request, bar_id=0, status=0):
	if request.method == 'GET':
		...
		drinkOrders = BarDrinkOrdered.objects.select_related("order").order_by("order__updated").filter(order__bar=bar_id).filter(
			order__current_status=status).reverse()[:6]
