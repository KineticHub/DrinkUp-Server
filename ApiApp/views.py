...
def VenuesNearLocation (request):
    if request.method == 'GET':
        ...
        for venue in all_venues:
            venue_point = (
                float(venue.latitude), float(venue.longitude))#Point(str(venue.latitude)+";"+str(venue.longitude))
...
def UserPasswordReset (request, template='registration/password_reset_email.html'):
    if request.method == 'POST':
        ...
        if email is not None:
            ...
            if form.is_valid():
                form.clean()
                form.save(from_email=settings.EMAIL_SENDER_PREFIX + " <team@letsdrinkup.com>",
