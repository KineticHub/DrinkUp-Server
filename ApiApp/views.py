...
def CreateNewOrder(request):
		...
		if request.method == 'POST':
			user_order = []
			user_order['appuser'] = request.user.appuser
			user_order['bar_id'] = request.POST.get('bar_id', None)
			user_order['tip_percent'] = request.POST.get('tip_percent', None)
			user_order['drinks'] = request.POST.get('drinks', None)

			new_order = createNewOrder( calculateOrder(user_order) )
...
def CalculateUserOrder(request):
    if request.method == 'POST':
            user_order = []
			user_order['appuser'] = request.user.appuser
			user_order['bar_id'] = request.POST.get('bar_id', None)
			...
			calculated_order = calculateOrder(user_order)

			response = json.dumps(calculated_order)
			return HttpResponse(response, mimetype="application/json")
