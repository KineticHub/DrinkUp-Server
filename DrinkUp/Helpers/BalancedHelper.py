...
class BalancedPaymentsHelper(object):
    def __init__(self):
    ...
    def addMerchantBankAccount(self, merchant, bank):
        ...
        bank_account = self.setupNewBankAccount(bank.routing_number, bank.account_number, bank.account_type,
                                                bank.bank_name)
    ...
    def addCreditCardToBuyer(self, account_uri, card_info):
        ...
        card = self.setupNewCreditCard(card_info['card_number'], card_info['expiration_month'],
                                       card_info['expiration_year'], card_info['security_code'])
    ...
    def captureHoldForOrder(self, order):
        ...
        debit = hold.capture(
            appears_on_statement_as=appears[:21],
            description=order.name + ' at ' + order.bar.venue.name,
        )
